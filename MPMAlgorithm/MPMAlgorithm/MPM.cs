...
namespace MPMAlgorithm
{
    public class MPM
    {
        ...
        public MPM(AdjacencyList adjacencyList, int source, int sink)
        {
        ...
        private int GetCapacity(int nodeFrom, int nodeTo, bool forward)
        {
            ...
            foreach (var neighbor in _graph._adjacencyList[nodeFrom])
            {
        ...
        private bool Bfs()
        {
            ...
            while (_queue.Count > 0)
            {
                ...
                foreach (var neighbor in _graph._adjacencyList[currentNode])
                {
        ...
        public int MaxFlow()
        {
            ...
            while (true)
            {
                ...
                foreach (var node in _graph._adjacencyList.Keys)
                {
                    _excessFlowSource[node] = 0;
                    _excessFlowSink[node] = 0;
                    foreach (var neighbor in _graph._adjacencyList[node])
                    {
